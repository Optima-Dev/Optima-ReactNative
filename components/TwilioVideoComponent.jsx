...
const TwilioVideoComponent = forwardRef((props, ref) => {
  const { token, roomName, identity } = props;

  function onRoomDidConnect() {
    setIsConnected(true);
    setStatus("Connected!");
  }

  function onRoomDidDisconnect() {
    setIsConnected(false);
    setStatus("Disconnected");
  }

  function onRoomDidFailToConnect(error) {
    console.error("Failed to connect to room:", error);
    setStatus("Disconnected");
  }

  function onParticipantAddedVideoTrack() {
    console.log("Participant added video track");
    setStatus("Volunteer joined the call");
  }

  function onParticipantRemovedVideoTrack() {
